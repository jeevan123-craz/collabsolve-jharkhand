...
import { MapPin, AlertTriangle, Clock, ThumbsUp, Send, Loader2, Sparkles, Bot, Paperclip, FileText, Film, X, Image as ImageIcon, Mic, MicOff, MessageSquare } from 'lucide-react';
import { db } from '@/lib/firebase';
import { collection, addDoc, serverTimestamp, query, orderBy, limit, onSnapshot, doc, updateDoc } from '@/lib/mock-firebase';
...
export default function Page() {
  ...
  const [assistantMessage, setAssistantMessage] = useState("Hi! I'm your Voice AI assistant. Click the microphone icon to describe your issue out loud, and I'll fill the form for you!");
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      setAssistantMessage(`You said: "${transcript}"\n\nThinking...`);
      setIsRecording(false);
      
      try {
        const res = await fetch('/api/ai/citizen-assistant', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            transcript,
            currentFormState: { title, description, district, category }
          })
        });
        ...
        if (data.updated_fields) {
          if (data.updated_fields.title && data.updated_fields.title !== "...") setTitle(data.updated_fields.title);
          if (data.updated_fields.description && data.updated_fields.description !== "...") setDescription(data.updated_fields.description);
          if (data.updated_fields.district && data.updated_fields.district !== "...") setDistrict(data.updated_fields.district);
          if (data.updated_fields.category && data.updated_fields.category !== "...") setCategory(data.updated_fields.category);
        }
        
        setAssistantMessage(data.bot_response || "Got it!");
        
        if ('speechSynthesis' in window && data.bot_response) {
          const utterance = new SpeechSynthesisUtterance(data.bot_response);
          window.speechSynthesis.speak(utterance);
        }
        
      } catch (err) {
        setAssistantMessage("Sorry, I had trouble processing that.");
      }
    };
    ...
    <div className="pt-8 px-gutter max-w-container-max mx-auto pb-xl">
      ...
      >
        ...
        <motion.section variants={itemVariants} className="w-full md:w-3/5 flex flex-col gap-md">
          ...
          {/* AI Voice Mode Link */}
          <Link href="/citizen/voice" className="group bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-all relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none"></div>
            <div className="flex flex-col gap-1 relative z-10">
              <div className="flex items-center gap-2 text-blue-700 font-bold text-lg">
                <Bot className="w-6 h-6" />
                Prefer speaking? Use Voice Mode
              </div>
              <p className="text-sm text-gray-600">Our Gemini 2.5 AI will listen to your issue and automatically fill this form for you.</p>
            </div>
            <div className="bg-blue-600 text-white rounded-full p-4 group-hover:scale-105 group-hover:bg-blue-700 transition-all shadow-md relative z-10 flex-shrink-0">
              ...
              <div className="flex flex-col gap-3 mt-2">
                <div className="flex justify-between items-center w-full">
                  <div className="flex items-center gap-2">
                    <label htmlFor="fileUpload" className="cursor-pointer text-xs font-bold text-primary flex items-center gap-1 hover:underline border border-primary/20 px-3 py-1.5 rounded-full bg-primary/5 transition-colors">
                      <Paperclip className="w-4 h-4" />
                      Attach Files
                    </label>
                    <input id="fileUpload" type="file" multiple accept="image/*,video/*,.pdf,.doc,.docx" onChange={handleFileUpload} className="hidden" />
                    {attachments.length > 0 && <span className="text-xs text-green-600 font-medium">{attachments.length} file(s) attached</span>}
                  </div>
                ...
                {attachments.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {attachments.map(att => (
                      <div key={att.id} className="relative group w-16 h-16 rounded-md border border-outline-variant overflow-hidden bg-surface-variant/30 flex items-center justify-center" title={att.name}>
                        {att.type === 'image' ? (
                          <img src={att.url} alt={att.name} className="w-full h-full object-cover" />
                        ) : att.type === 'video' ? (
                          <Film className="w-6 h-6 text-on-surface-variant" />
                        ) : (
                          <FileText className="w-6 h-6 text-on-surface-variant" />
                        )}
                        <button type="button" onClick={() => setAttachments(prev => prev.filter(a => a.id !== att.id))} className="absolute top-0.5 right-0.5 bg-error text-on-error rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    ))}
