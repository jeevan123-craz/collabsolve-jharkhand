...
{/* Project Milestones */}
<section className="bg-surface-container-lowest rounded-lg p-md shadow-sm border border-surface-variant">
  <h2 className="font-headline-md text-headline-md text-on-surface mb-md flex items-center gap-xs">
    <span className="material-symbols-outlined text-primary">flag</span>
    Milestones
  </h2>
  <div className="flex flex-col gap-4 relative">
    <div className="absolute left-3 top-2 bottom-2 w-0.5 bg-surface-variant z-0" />
    
    {[
      { title: 'Project Kickoff', date: 'Oct 15', status: 'completed' },
      { title: 'Prototype Development', date: 'Nov 10', status: 'in-progress' },
      { title: 'Field Testing', date: 'Dec 05', status: 'pending' },
      { title: 'Final Review', date: 'Jan 20', status: 'pending' }
    ].map((m, i) => (
      <div key={i} className="flex gap-4 relative z-10 items-start">
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 bg-surface-container-lowest ${m.status === 'completed' ? 'border-success-green' : m.status === 'in-progress' ? 'border-primary' : 'border-outline-variant'}`}>
          {m.status === 'completed' && <span className="material-symbols-outlined text-[14px] text-success-green">check</span>}
          {m.status === 'in-progress' && <span className="w-2 h-2 rounded-full bg-primary" />}
        </div>
        <div>
          <h4 className="font-label-md text-on-surface">{m.title}</h4>
